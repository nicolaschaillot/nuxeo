            ...
            for (DocumentModel recordDoc : loadDocuments(session, ids)) {
                ...
                if (!record.isRetainUntilInderterminate()) {
                    log.debug("Record {} has already a determinate retention date {}, ignoring ...",
                            ...
                            () -> RetentionConstants.DEFAULT_DATE_FORMAT.format(recordDoc.getRetainUntil()));
                    continue;
                }
                ...
                session.setRetainUntil(recordDoc.getRef(), record.getRule(session).getRetainUntilDateFromNow());
            }
        }
    }
